...
import { PaymentSummery } from '../models/order';
...
export class PaymentCardPipe implements PipeTransform {
  ...
  transform(value?: ConfirmationToken['payment_method_preview'] | PaymentSummery, ...args: unknown[]): unknown {
    if (value && 'card' in value) {
      ...
      const { brand, last4, expMonth, expYear } = value as PaymentSummery;
